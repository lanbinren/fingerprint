...
static irqreturn_t sf_ctl_device_irq(int irq, void *dev_id)
{
    ...
    schedule_work(&sf_ctl_dev.work_queue);
	   ...
	   __pm_wakeup_event(&sf_ctl_dev.sf_wakelock, msecs_to_jiffies(5000));
#else
...
#ifdef CONFIG_PM_WAKELOCKS
        wakeup_source_init(&sf_ctl_dev.sf_wakelock ,"sf_wakelock");
#else
...
#if (defined(CONFIG_MTK_SPI) || SF_TRUSTKERNEL_COMPAT_SPI_MT65XX)
	printk("LPP---------yes-------spi config start\n");
...
xprintk(SF_LOG_LEVEL, "111111%s(..) enter.\n", __FUNCTION__);
...
#else
    /* not used */
    printk("LPP--------no--------spi not config end\n");
    int smt_conf;
#endif
    sf_ctl_dev.pdev->max_speed_hz = SF_DEFAULT_SPI_SPEED;
    sf_ctl_dev.pdev->bits_per_word = 8;
    ...
    do {
        ...
        xprintk(KERN_INFO, "000  writebuf: 0x%02x 0x%02x\n", writebuf[0],writebuf[1]);
        ret =  tee_spi_transfer(&smt_conf, cfg_len, writebuf, readbuf, 6);
        xprintk(KERN_INFO, "aaa  writebuf: 0x%02x 0x%02x\n", writebuf[0],writebuf[1]);

        if (ret != 0) {
            xprintk(KERN_ERR, "SPI transfer failed\n");
            continue;
        }
		xprintk(KERN_INFO, "11111 readbuf: 0x%02x%02x0x%02x%02x0x%02x\n", readbuf[2],readbuf[3],readbuf[4],readbuf[5],readbuf[6]);
        ...
        writebuf[3] = 0x02;
        writebuf[4] = 0x00;
        xprintk(KERN_INFO, "bbbb writebuf: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n", writebuf[0],writebuf[1],writebuf[2],writebuf[3],writebuf[4]);
		...
		xprintk(KERN_INFO, "22222 readbuf: 0x%02x%02x0x%02x%02x0x%02x\n", readbuf[2],readbuf[3],readbuf[4],readbuf[5],readbuf[6]);
        if (readbuf[5] == 0x82) {
            xprintk(KERN_INFO, "read chip is ok\n");
            return 0;
        }
		...
		xprintk(KERN_INFO, "3333 readbuf: 0x%02x%02x0x%02x%02x0x%02x\n", readbuf[2],readbuf[3],readbuf[4],readbuf[5],readbuf[6]);
        if (readbuf[4] == 0x82) {
            xprintk(KERN_INFO, "read chip is ok\n");
            return 0;
        }
...
#else
#if SF_REG_DEVICE_BY_DRIVER
    sf_device = platform_device_alloc("sunwave_fp", 0);
